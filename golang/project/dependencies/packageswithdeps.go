...
func (pwd *PackageWithDeps) PopulateModAndPublish(targetRepo string, cache *golang.DependenciesCache, serviceManager *artifactory.ArtifactoryServicesManager) error {
	var path string
	var err error
	...
	logError(err)
	return pwd.publishDependencyAndPopulateTransitive(path, targetRepo, output, cache, serviceManager)
}

// Updating the new mod content
func (pwd *PackageWithDeps) updateModContent(path string, cache *golang.DependenciesCache) error {
	if path != "" {
...
func (pwd *PackageWithDeps) publishDependencyAndPopulateTransitive(path, targetRepo string, graphDependencies map[string]bool, cache *golang.DependenciesCache, serviceManager *artifactory.ArtifactoryServicesManager) error {
	...
	if !published && (pwd.patternMatched(pwd.regExp.GetNotEmptyModRegex()) || pwd.patternMatched(pwd.regExp.GetEditedByJFrogCli())) {
	...
	err = pwd.updateCacheAndPublishDependency(path, targetRepo, cache, serviceManager)
	logError(err)
...
func (pwd *PackageWithDeps) prepareAndPublish(targetRepo string, cache *golang.DependenciesCache, serviceManager *artifactory.ArtifactoryServicesManager) error {
...
func (pwd *PackageWithDeps) updateCacheAndPublishDependency(path, targetRepo string, cache *golang.DependenciesCache, serviceManager *artifactory.ArtifactoryServicesManager) error {
	dependenciesMap := cache.GetMap()
	published, _ := dependenciesMap[pwd.Dependency.GetId()]
	if !published {
		if pwd.tidyEnum.GetTidyValue() == pwd.tidyEnum.GetRecursiveTidyOverwrite() {
...
func (pwd *PackageWithDeps) updateModWithoutIndirect(path string, cache *golang.DependenciesCache) {
...
func (pwd *PackageWithDeps) setTransitiveDependencies(targetRepo string, graphDependencies map[string]bool, cache *golang.DependenciesCache, auth auth.ArtifactoryDetails) {
...
func (pwd *PackageWithDeps) populateTransitive(targetRepo string, cache *golang.DependenciesCache, serviceManager *artifactory.ArtifactoryServicesManager) {
	...
	for _, transitiveDep := range pwd.transitiveDependencies {
		...
		if !published {
			...
			err := transitiveDep.PopulateModAndPublish(targetRepo, cache, serviceManager)
			logError(err)
		} else {
